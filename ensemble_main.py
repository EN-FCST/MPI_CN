...
def main(delta_day, day0, key):
    ...
    for key, val in dict_var.items():
        for fcst_key in fcst_keys:
            #print(key)
            #print(fcst_key)
            #print(val[fcst_key].shape)
            dict_interp[key][fcst_key] = mt.grid_transfer(dict_latlon[key][fcst_key][0], dict_latlon[key][fcst_key][1], val[fcst_key], lon, lat)
